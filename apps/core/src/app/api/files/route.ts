...
export async function GET(req: Request) {
  ...
  // Folders: list distinct folders or subfolders under a parent path
  if (action === "folders") {
    ...
    if (parent) {
      // Only return direct subfolders under parent
      const prefix = parent.endsWith("/") ? parent : parent + "/";
      const children = allFolders.filter((f) => f.startsWith(prefix) && f !== parent);
      const direct = children.filter((f) => {
        const rest = f.slice(prefix.length);
        return !rest.includes("/");
      });
      return NextResponse.json({ folders: direct, parent });
    }
