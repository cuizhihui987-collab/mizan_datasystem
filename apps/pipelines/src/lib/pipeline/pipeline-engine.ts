...
import { topologicalSort, buildIncomingMap, getInputTableNames } from "@/lib/pipeline/dag-utils";
...
export class PipelineEngine {
  ...
  async executeDAG(pipelineId: string): Promise<ExecutionResult> {
    ...
    const edges: { source: string; target: string }[] = (() => {
      try { return JSON.parse(pipeline.edges || "[]"); }
      catch { return []; }
    })();

    // 如果 DAG 没有边，退化为线性执行
    if (edges.length === 0) {
      return this.execute(pipelineId);
    }

    // 拓扑排序
    const execOrder = topologicalSort(pipeline.steps as unknown as any[], edges as any);
    if (execOrder.length === 0) {
      return { success: false, pipelineStatus: "FAILED", stepResults: [], error: "流程中存在循环依赖" };
    }

    // 构建步骤 Map
    const stepMap = new Map(pipeline.steps.map((s) => [s.id, s]));
    const resultMap = new Map<string, string>(); // stepId → outputPhysicalName
    ...
    try {
      // 按拓扑序逐批执行
      const incomingMap = buildIncomingMap(edges as any);
      ...
      for (const stepId of execOrder) {
        ...
        // 收集输入表 (来自上游)
        const inputTables = (incomingMap.get(stepId) || [])
          .map((srcId: string) => resultMap.get(srcId))
          .filter(Boolean) as string[];

        // 跳过 flow 控制节点 (它们不产生数据表)
        if (["flow_branch", "flow_switch", "flow_parallel", "flow_loop"].includes(step.stepType)) {
          // 标记为 COMPLETED, 透传第一个输入表
          const passThroughTable = inputTables[0] || null;
          if (passThroughTable) resultMap.set(stepId, passThroughTable);
          ...
          stepResults.push({
            ...
            status: "COMPLETED",
            outputTable: passThroughTable || undefined,
          });
          continue;
        }

        // flow_merge_all: UNION ALL 多个输入
        if (step.stepType === "flow_merge_all") {
          await prisma.pipelineStep.update({
            where: { id: step.id },
            data: { status: "RUNNING", startedAt: new Date() },
          });
          try {
            const result = await this.executeMergeAll(step, inputTables);
            resultMap.set(stepId, step.outputPhysicalName);
            await prisma.pipelineStep.update({
              where: { id: step.id },
            ...
            await this.handleStepError(step, error, stepResults, pipelineId);
            return { success: false, pipelineStatus: "FAILED", stepResults };
          }
          continue;
        }

        // 普通步骤: 使用第一个输入表作为 previousTableName
        const previousTableName = inputTables[0] || null;
        ...
        try {
          ...
          await this.handleStepError(step, stepError, stepResults, pipelineId);
          return { success: false, pipelineStatus: "FAILED", stepResults };
        }
      }
  ...
  // ── flow_merge_all: UNION ALL 合并 ────────────────────────
  private async executeMergeAll(
    step: { outputPhysicalName: string },
    inputTables: string[]
  ): Promise<{ affectedRows: number }> {
    if (inputTables.length < 2) {
      throw new Error(`合并流需要至少 2 个输入，当前只有 ${inputTables.length} 个`);
    }

    const outName = safeIdentifier(step.outputPhysicalName);
    await prisma.$executeRawUnsafe(`DROP TABLE IF EXISTS "${outName}"`);

    const first = safeIdentifier(inputTables[0]);
    const rest = inputTables.slice(1).map((t) => safeIdentifier(t));

    // CREATE TABLE AS SELECT * FROM table1 UNION ALL SELECT * FROM table2 ...
    const selects = [first, ...rest].map((t) => `SELECT * FROM "${t}"`);
    const sql = `CREATE TABLE "${outName}" AS ${selects.join(" UNION ALL ")}`;
  ...
  private async handleStepError(
    step: { id: string; label?: string | null; stepType: string },
    error: unknown,
